...
class AutoVASBrain:
    ...
    def baixar_relatorio(self, relatorio_id, senha_sudo: str, id_container: str, senha_openvas):
        ...
        '''
        self.exec_script_temp(script_parte_1, senha_sudo)
        print("Relatório XML gerado com sucesso.")

        # Parte 2: conversão para Excel
        script_parte_2 = f'''
        docker exec -it {id_container} bash -c "source /path/to/venv/bin/activate && cd auto_vas && openvasreporting -i pretty_relatorio.xml"
        '''
        self.exec_script_temp(script_parte_2, senha_sudo)
        print("Conversão para Excel concluída.")

        # Parte 3: copiar o arquivo
        script_parte_3 = f'''
        docker cp {id_container}:/auto_vas/openvas_report.xlsx "{caminho_arquivo}"
        '''
        self.exec_script_temp(script_parte_3, senha_sudo)
        print(f"Relatório final salvo em: {caminho_arquivo}")
